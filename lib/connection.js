...
// NUEVA FUNCIÓN: Destruye la sesión fantasma si no está registrada por completo
async function forceCleanSession(sessionPath) {
    ...
    if (fs.existsSync(credsPath)) {
        try {
            ...
            if (!creds.registered) {
                console.log(chalk.yellow('🧹 Forzando limpieza de sesión incompleta para evitar el Error 428...'))
...
export async function start() {
    ...
    // Ejecutamos la limpieza extrema ANTES de cargar cualquier estado
    await forceCleanSession(sessionPath)
    ...
    if (usePairingCode && !state.creds.registered) {
        phoneNumber = await question(chalk.cyan('Introduce tu número de teléfono (ej: 521...): '))
    ...
    sock = makeWASocket({
        ...
        browser: ['Ubuntu', 'Chrome', '111.0.5563.146'],
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr && !pairingCodeRequested && usePairingCode && !state.creds.registered && phoneNumber) {
            pairingCodeRequested = true
            // Esperamos 2 segundos exactos para que el websocket se estabilice
            setTimeout(async () => {
                try {
                    const code = await sock.requestPairingCode(phoneNumber)
                    console.log(chalk.white('\n🔑 Tu código de vinculación es: ') + chalk.bold.green(code) + '\n')
                } catch (error) {
                    console.error(chalk.red('❌ Error al solicitar el código:'), error.message)
                }
            }, 2000)
        }
        ...
        if (connection === 'close') {
            ...
            if (statusCode === 515 || statusCode === 428) {
                ...
                if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
                    console.log(chalk.red('❌ Demasiados intentos. Borrando sesión por seguridad...'))
                    try {
                        fs.rmSync(sessionPath, { recursive: true, force: true })
                    } catch(e) {}
                    reconnectAttempts = 0
