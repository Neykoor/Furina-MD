...
async function forceCleanSession(sessionPath) {
    ...
    if (fs.existsSync(credsPath)) {
        try {
            ...
            if (!creds.registered) {
                console.log(chalk.yellow('🧹 Forzando limpieza de sesión incompleta...'))
...
process.on('uncaughtException', (err) => {
    if (err.output?.statusCode === 428) {
        console.error(chalk.yellow('⚠️ Error de fondo por conexión cerrada (428).'))
    } else {
        console.error(chalk.red('⚠️ Excepción no capturada:'), err.message || err)
    }
})
