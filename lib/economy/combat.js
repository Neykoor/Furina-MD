...
export const JEFES = {
    goblin:      { nombre: 'Jefe Goblin',      vida: 200,  danio: 25,  defensa: 10, exp: 100,  money: 500,  drops: { oro: 2, hierro: 5 } },
    ogro:        { nombre: 'Ogro Salvaje',     vida: 500,  danio: 50,  defensa: 20, exp: 300,  money: 1500, drops: { oro: 5, rubi: 1 } },
    troll:       { nombre: 'Troll de Cueva',   vida: 800,  danio: 70,  defensa: 30, exp: 500,  money: 3000, drops: { esmeralda: 2, hierro: 10 } },
    gigante:     { nombre: 'Gigante de Piedra', vida: 1500, danio: 100, defensa: 50, exp: 1000, money: 7000, drops: { diamante: 1, titanio: 2 } },
    dragon_anciano: { nombre: 'Dragon Anciano', vida: 3000, danio: 200, defensa: 80, exp: 3000, money: 20000, drops: { diamante: 5, obsidiana: 3, oro: 20 } },
    demonio:     { nombre: 'Senor Demonio',    vida: 5000, danio: 300, defensa: 100, exp: 5000, money: 50000, drops: { diamante: 10, meteorito: 1, titanio: 5 } },
}
...
export function getCombatStats(username) {
    ...
    const baseDanio = 10 + (level * 2)
    const baseDefensa = 5 + (level * 1)

    return {
        vida: Math.floor(baseVida + equipBonus.defensa * 2),
        danio: Math.floor(baseDanio + equipBonus.danio),
        defensa: Math.floor(baseDefensa + equipBonus.defensa),
        critChance: Math.min(5 + (level * 0.3), 50),
        critMultiplier: 2.0,
        evasion: Math.min(2 + (level * 0.2), 30),
    }
}
...
export function huntBattle(username, animalId) {
    ...
    while (playerVida > 0 && animalVida > 0 && turnos < 50) {
        ...
        const danioJugador = Math.max(1, Math.floor(
            (playerStats.danio * (crit ? playerStats.critMultiplier : 1)) - (animal.defensa || animal.vida * 0.05)
        ))
        animalVida -= danioJugador
        ...
        if (!evade) {
            // FIX: Usar animal.danio si existe, fallback a vida*0.1 para compatibilidad
            const animalDanio = animal.danio || Math.floor(animal.vida * 0.1)
            const danioAnimal = Math.max(1, Math.floor(
                animalDanio - (playerStats.defensa * 0.3)
            ))
            playerVida -= danioAnimal
    ...
    if (victory) {
        ...
        if (Math.random() < 0.1 && animal.rareza === 'legendario') {
            itemsDropped.push({ item: 'hueso_dragon', cantidad: 1 })
        }
    } else {
...
export function bossBattle(username, bossId) {
    ...
    while (playerVida > 0 && jefeVida > 0 && turnos < 100) {
        ...
        const crit = Math.random() * 100 < playerStats.critChance
        const danioJugador = Math.max(1, Math.floor(
            (playerStats.danio * (crit ? playerStats.critMultiplier : 1)) - (jefe.defensa * 0.5)
        ))
        jefeVida -= danioJugador

        log.push({ turno: turnos, actor: 'player', danio: danioJugador, crit, targetVida: Math.max(0, jefeVida) })
        ...
        if (!evade) {
            const danioJefe = Math.max(1, Math.floor(
                (jefe.danio * (special ? 1.5 : 1)) - (playerStats.defensa * 0.4)
            ))
            playerVida -= danioJefe
    ...
    if (victory) {
        ...
        if (Math.random() < 0.2) {
            const bonusItems = ['pocion_vida', 'pocion_fuerza', 'cebo_magico']
