...
// FIX: Soporte para misiones de tipo 'mixto'
export function updateMissionProgress(username, accion, cantidad = 1, itemId = null) {
    ...
    const checkMission = (mission) => {
        ...
        // FIX: Soporte para misiones mixto
        if (obj.accion === 'mixto') {
            if (obj[accion] !== undefined) {
                mission.mixProgress = mission.mixProgress || {}
                mission.mixProgress[accion] = (mission.mixProgress[accion] || 0) + cantidad

                // Verificar si todas las actividades están completas
                const actividades = Object.keys(obj).filter(k => k !== 'accion')
                const allComplete = actividades.every(k => (mission.mixProgress[k] || 0) >= obj[k])

                if (allComplete && !mission.completed) {
...
`
    ...
    for (const m of freshMissions.weekly) {
        const status = m.claimed ? '✅' : m.completed ? '💰' : '⏳'
        // FIX: Soporte para barra de progreso en misiones mixto
        let progress = m.progress
        let total = m.objetivo.cantidad
        if (m.objetivo.accion === 'mixto' && m.mixProgress) {
            const actividades = Object.keys(m.objetivo).filter(k => k !== 'accion')
            const completadas = actividades.filter(k => (m.mixProgress[k] || 0) >= m.objetivo[k]).length
