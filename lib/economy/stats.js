...
// FIX: data.item puede ser string (ID) u objeto
export function updateStats(username, actividad, data = {}) {
    ...
    if (data.item) {
        // FIX: Resolver item si es string
        const itemObj = typeof data.item === 'string' ? getItem(data.item) : data.item
        const itemName = itemObj?.id || data.item
        ...
        const itemValue = itemObj?.valor || 0
        if (!stats[actividad].best || itemValue > (stats[actividad].best?.valor || 0)) {
            stats[actividad].best = itemObj || data.item
        }
    }
