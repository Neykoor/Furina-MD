...
import { getOrCreateUser, loadUsers, saveUsers } from './users.js'
...
// FIX CRÍTICO: La función anterior tenía `await import()` dentro de una función
// síncrona (sin async), lo que causa SyntaxError en Node.js ESM al arrancar.
// Ahora usamos saveUsers de users.js que ya maneja la persistencia correctamente.
function saveFamilySync(userId, family) {
    const uid = String(userId).replace(/[^0-9]/g, '') || userId
...
export function getFamilyStats(userId) {
    ...
    while (getFamilyData(tempId).parents.length > 0 && !visitedGen.has(tempId)) {
        visitedGen.add(tempId)
        generation++
        tempId = getFamilyData(tempId).parents[0]
        if (generation > 20) break // seguro anti-ciclos
    }
