...
export async function handler(conn, chat) {
  try {
    ...
    // Verificar si es un sub-bot y obtener el ID correcto
    const isSubBot = conn.isSubBot || false
    ...
    // Clave única incluyendo el bot que lo procesa
    ...
    const isUserPremium = isPremium(sender) || isSubbotPremium(sender) || isSubbotPremium(botId)
    const userType = getUserType(sender)
    
    // Aplicar config premium si es sub-bot premium
    if (isUserPremium || isSubbotPremium(botId)) {
      applyPremiumConfig(isSubbotPremium(botId) ? botId : sender)
    }
