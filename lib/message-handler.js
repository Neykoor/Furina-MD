...
const cooldownCache = new NodeCache({ stdTTL: 10 })
...
async function dfail(tipo, m, conn, cmd = '') {
    const mensajes = {
        ...
        cooldown: '⏳ Espera un momento antes de usar otro comando.',
        disabled: '🔒 Este comando está deshabilitado en este grupo.'
    }
    ...
    const emoji = { owner: '👑', group: '👥', private: '👤', admin: '🛡️', botAdmin: '🤖', invalid: '', cooldown: '⏳', disabled: '🔒' }[tipo] || '🚫'
...
// Anti-spam check
function checkCooldown(sender, cooldownMs = 3000) {
    const key = `cooldown_${sender}`
    if (cooldownCache.has(key)) {
        return false
    }
    cooldownCache.set(key, true, cooldownMs / 1000)
    return true
}
...
export async function handleMessage(sock, m, commands) {
    try {
        ...
        // ═══════════════════════════════════════════════════════════════
        // 🛡️ ANTI-LINK DETECTOR (INTEGRACIÓN)
        // ═══════════════════════════════════════════════════════════════
        try {
            ...
            // Silenciar errores del antilink para no afectar el bot
        }
        // ═══════════════════════════════════════════════════════════════

        // Chatbot mode - respond to mentions or specific patterns
        ...
        if (isGroup) {
            try {
                const groupConfig = global.db?.data?.chats?.[from]
                if (groupConfig?.botDisabled) {
                    // Only respond to enable command or owner
                    if (!text.startsWith(usedPrefix + 'bot') && !checkOwner(senderJid)) {
                        return
                    }
                }
            } catch { }
        ...
        if (!text || !text.startsWith(usedPrefix)) {
            // Chatbot response for mentions without command
            if (isMentioned && text.length > 0) {
                try {
                    const { chatbotResponse } = await import('../src/commands/funciones/chatbot.js')
                    await chatbotResponse(sock, m, text)
                } catch { }
            }
            return
        }
        ...
        const cooldownMs = subConfig.cooldown || 3000
        if (!checkCooldown(senderJid, cooldownMs) && !isOwner) {
            await dfail('cooldown', m, sock)
            return
        }
