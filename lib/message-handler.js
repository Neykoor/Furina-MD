...
async function handleListResponse(sock, m, commands) {
    try {
        let selected = m.message?.listResponseMessage?.title ||
            m.message?.listResponseMessage?.singleSelectReply?.selectedRowId ||
            m.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson

        if (!selected) return false

        // --- FIX: paramsJson viene como string JSON en nativeFlow, hay que parsearlo ---
        let selectedId = selected
        if (typeof selected === 'string' && selected.startsWith('{')) {
            try {
                const parsed = JSON.parse(selected)
                selectedId = parsed.id || parsed.title || selected
            } catch (e) {
                // Si no parsea, usar el string original
            }
        }

        let command = ''
        const check = String(selectedId).toLowerCase()

        if (check.startsWith('.')) {
            command = check.slice(1).split(' ')[0]
        } else if (check.includes('menú gacha') || check.includes('menu gacha')) {
            command = 'gacha'
        } else if (check.includes('menú grupo') || check.includes('menu grupo')) {
            command = 'grupo'
        } else if (check.includes('menú antilinks') || check.includes('menu antilinks')) {
            command = 'antilinks'
        } else if (check.includes('menú rpg') || check.includes('menu rpg')) {
            command = 'rpg'
        } else if (check.includes('menú games') || check.includes('menu games')) {
            command = 'games'
        } else if (check.includes('configurar')) {
            command = 'config'
        } else if (check.includes('reiniciar')) {
            command = 'restartbot'
        }
