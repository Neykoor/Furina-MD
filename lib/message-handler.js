...
function checkOwner(sender) {
    ...
    return owners.some(o => {
        const ownerNum = StringNum = String(Array.isArray(o) ? o[0] : o).replace(/[^0-9]/g, '')
        return ownerNum === num
    })
}
...
// Soporta: 123@s.whatsapp.net, 123:1@s.whatsapp.net, 123@lid, etc.
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// Compara dos JIDs ignorando formato (device ID, dominio, LID)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// Obtiene el JID del bot desde múltiples fuentes posibles
// ══════════════════════════════════════════════════════════════════
function getBotJid(sock) {
    // Intentar múltiples propiedades donde Baileys guarda el JID del bot
    const candidates = [
        sock.user?.id,
        sock.user?.jid,
...
async function getAdminInfo(conn, groupJid, senderJid) {
    try {
        ...
        // Buscar al bot en participants comparando números puros
        let botP = null
        ...
        // Si no se encontró, intentar buscar por número puro de cada candidate
        if (!botP) {
            for (const candidate of botJids) {
        ...
        // DEBUG: Descomenta estas líneas para diagnosticar
        console.log(chalk.yellow('[DEBUG getAdminInfo]'))
        console.log('  botJids candidatos:', botJids)
        console.log('  botP encontrado:', botP ? { id: botP.id, admin: botP.admin } : 'NO ENCONTRADO')
        console.log('  senderJid:', senderJid)
        console.log('  userP encontrado:', userP ? { id: userP.id, admin: userP.admin } : 'NO ENCONTRADO')
        console.log('  Total participants:', participants.length)
        console.log('  Sample participants:', participants.slice(0, 3).map(p => ({ id: p.id, admin: p.admin })))
