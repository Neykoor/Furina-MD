...
// Cache de metadata de grupo: 30s para no saturar la API de WA
...
// ══════════════════════════════════════════════════════════════════
// Normaliza un JID: extrae solo el número base, eliminando :XX y dominio
// Ej: "123456789:1@s.whatsapp.net" → "123456789"
// Ej: "123456789@s.whatsapp.net"  → "123456789"
// Ej: "123456789@lid"             → "123456789"
// ══════════════════════════════════════════════════════════════════
function normalizeJid(jid) {
    if (!jid) return ''
    // Primero eliminamos todo desde @ en adelante
    let base = String(jid).split('@')[0]
    // Luego eliminamos todo desde : en adelante (el :XX de Baileys)
    base = base.split(':')[0]
...
async function getAdminInfo(conn, groupJid, senderJid) {
    try {
        ...
        // Obtener JID del bot de múltiples fuentes posibles
        const botJid = conn.user?.id || conn.user?.jid || ''
        const botLid = conn.user?.lid || ''
        
        const botNum = normalizeJid(botJid)
        const botLidNum = normalizeJid(botLid)
        ...
        // Debug (quitar en producción si no lo necesitas)
        // console.log('[DEBUG] botJid raw:', botJid, '| botNum:', botNum)
        // console.log('[DEBUG] botP found:', botP)
        // console.log('[DEBUG] participants sample:', participants.slice(0, 3).map(p => p.id))
