...
// FIX: El framework (Baileys + LIDSync) ya resuelve m.isBotAdmin.
// Usamos eso como fuente PRINCIPAL. Fallback: buscar en participants.
// ══════════════════════════════════════════════════════════════════
async function getAdminInfo(conn, groupJid, senderJid, m) {
    try {
        ...
        // ─── isBotAdmin: PRIORIDAD #1 ───
        // El framework YA resolvió esto correctamente (lo vimos en M4 del comando)
        // Si m.isBotAdmin existe y es true, lo usamos directo
        let isBotAdmin = m?.isBotAdmin === true

        // Fallback: solo si m.isBotAdmin no existe o es false, buscamos manual
        if (!isBotAdmin) {
