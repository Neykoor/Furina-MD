// lib/serbot.js - ADAPTADO PARA WEB + WHATSAPP
...
// ═══════════════════════════════════════════════════════════════════
// WEB CALLBACKS - Para integración con lib/web.js
// ═══════════════════════════════════════════════════════════════════
if (!global.webCallbacks) global.webCallbacks = new Map()
// Estructura: requestId -> { resolve, reject, type: 'qr'|'code', timestamp }

export function registerWebCallback(requestId, callbacks) {
    global.webCallbacks.set(requestId, {
        ...callbacks,
        timestamp: Date.now()
    })
    // Auto-cleanup después de 2 minutos
    setTimeout(() => {
        const cb = global.webCallbacks.get(requestId)
        if (cb && !cb.resolved) {
            cb.reject(new Error('Timeout'))
            global.webCallbacks.delete(requestId)
        }
    }, 120000)
}

export function resolveWebCallback(requestId, data) {
    const cb = global.webCallbacks.get(requestId)
    if (cb && !cb.resolved) {
        cb.resolved = true
        cb.resolve(data)
...
export function rejectWebCallback(requestId, error) {
    const cb = global.webCallbacks.get(requestId)
    if (cb && !cb.resolved) {
        cb.resolved = true
...
// ═══════════════════════════════════════════════════════════════════
// ENVIAR CÓDIGO CON BOTÓN (WhatsApp)
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
// CREAR SUB-BOT - VERSIÓN UNIFICADA (WhatsApp + Web)
// ═══════════════════════════════════════════════════════════════════
export async function createSubBot(options = {}) {
    const {
        sessionPath,
        m = null,              // Mensaje de WhatsApp (null si es desde web)
        conn = null,           // Conexión de WhatsApp (null si es desde web)
        userId,
        isAutoStart = false,
        mcode = false,
        savedConfig = null,
        // NUEVOS PARÁMETROS PARA WEB:
        fromWeb = false,       // true si viene de la web
        webRequestId = null,   // ID para callback a la web
        webCallbacks = null    // { onQR, onCode, onConnected, onError }
    } = options
    ...
    if (isSubBotConnected(uid)) {
        ...
        // Si viene de web, notificar error
        if (fromWeb && webRequestId) {
            rejectWebCallback(webRequestId, 'Este número ya tiene un bot activo')
        }
        return
    }

    // Verificar límite de sub-bots por usuario (solo WhatsApp)
    if (!isAutoStart && ownerSender && !fromWeb) {
    ...
    try {
        ...
        if (fromWeb && webRequestId) {
            rejectWebCallback(webRequestId, 'Error al iniciar sesión')
        }
        return
    }
    ...
    // Timer de conexión (solo WhatsApp, la web maneja su propio timeout)
    if (!isAutoStart && !fromWeb) {
    ...
    // Timer para web (más corto)
    if (fromWeb && !isAutoStart) {
        connectionTimer = setTimeout(async () => {
            if (!isConnected) {
                await cleanup(true)
                if (webRequestId) {
                    rejectWebCallback(webRequestId, 'Tiempo de conexión agotado')
                }
            }
        }, CONNECTION_TIMEOUT)
    }
    ...
    async function handlePairingCode() {
        try {
            ...
            // Guardar en global para referencia
            ...
            // Si viene de WhatsApp
            if (!fromWeb && conn && m && m.chat) {
                await conn.sendMessage(m.chat, {
                    image: { url: global.logo || 'https://files.catbox.moe/gptlxc.jpg' },
                    caption: MSG_CODE
                }, { quoted: m })
            ...
            // Si viene de Web
            if (fromWeb) {
                if (webCallbacks?.onCode) {
                    webCallbacks.onCode(formatted, secret)
                }
                if (webRequestId) {
                    resolveWebCallback(webRequestId, {
                        type: 'code',
                        code: formatted,
                        rawCode: secret,
                        expiresIn: 60
            ...
            if (!fromWeb && conn && m && m.chat && !isAutoStart) {
                await conn.sendMessage(m.chat, {
            ...
            if (fromWeb && webRequestId) {
                rejectWebCallback(webRequestId, error.message)
            }
    ...
    async function handleQRCode() {
        try {
            ...
            // Si viene de Web
            if (fromWeb) {
                if (webCallbacks?.onQR) {
                    webCallbacks.onQR(currentQR)
                }
                if (webRequestId) {
                    // Generar buffer QR para la web
                    const qrBuffer = await qrcode.toBuffer(currentQR, {
                        scale: 8,
                        margin: 2,
                        errorCorrectionLevel: 'H',
    ...
    async function handleOpenConnection() {
        ...
        if (!isAutoStart && !fromWeb && conn && m && m.chat) {
            await conn.sendMessage(m.chat, {
        ...
        // Notificación Web
        if (fromWeb) {
            if (webCallbacks?.onConnected) {
                webCallbacks.onConnected({
                    jid: userJid,
                    name: userName,
                    number: userNum
                })
...
// ═══════════════════════════════════════════════════════════════════
// FUNCIÓN ESPECÍFICA PARA WEB: Crear sub-bot y retornar Promise
// ═══════════════════════════════════════════════════════════════════
export async function createSubBotForWeb(phoneNumber, type = 'qr') {
    const uid = cleanNum(phoneNumber)
    const sessionPath = path.join(SUBBOT_FOLDER, uid)
    const requestId = `web_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`

    return new Promise(async (resolve, reject) => {
        // Registrar callback para la web
        registerWebCallback(requestId, { resolve, reject })
        ...
        try {
            await createSubBot({
                ...
                webCallbacks: {
                    onQR: (qr) => {
                        console.log(chalk.cyan(`🌐 QR generado para web: ${uid}`))
                    },
                    onCode: (code, raw) => {
                        console.log(chalk.cyan(`🌐 Código generado para web: ${uid} -> ${code}`))
                    },
                    onConnected: (info) => {
                        console.log(chalk.green(`🌐 Sub-bot conectado desde web: ${info.name}`))
                    }
                }
            })
        } catch (error) {
            rejectWebCallback(requestId, error.message)
        }
    })
}

// ═══════════════════════════════════════════════════════════════════
// FUNCIÓN ESPECÍFICA PARA WEB: Obtener QR actual
// ═══════════════════════════════════════════════════════════════════
export function getCurrentQR(userId) {
    const uid = cleanNum(userId)
    const sent = global.sentCodes.get(uid)
    if (sent && sent.type === 'qr' && sent.qr) {
        return sent.qr
    }
    return null
}
...
// ═══════════════════════════════════════════════════════════════════
// FUNCIÓN ESPECÍFICA PARA WEB: Obtener código actual
// ═══════════════════════════════════════════════════════════════════
export function getCurrentCode(userId) {
    const uid = cleanNum(userId)
    const sent = global.sentCodes.get(uid)
    if (sent && sent.type === 'code' && sent.code) {
        return {
            formatted: sent.formatted,
            raw: sent.code,
            expiresAt: sent.timestamp + CODE_EXPIRY
        }
    }
    return null
}
