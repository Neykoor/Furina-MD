...
export function validateTokenFormat(token) {
    return /^ASTA-[A-Z0-9]{5}$/.test(token)
}

// ==================== BOT STATUS HELPERS ====================
export function getBotDetailedStatus(jid) {
    const num = cleanNum(jid)
    const sock = global.subBots.get(jid) || global.subBots.get(num) || global.subBots.get(`${num}@s.whatsapp.net`)

    if (!sock) return null

    const config = sock.subConfig || getSubConfig(jid)
    const wsState = sock.ws?.readyState

    return {
        jid: sock.user?.jid || jid,
        name: sock.user?.name || config.name || 'Unknown',
        number: num,
        status: wsState === 1 ? 'connected' : 'disconnected',
        uptime: process.uptime(),
        config: config,
        groups: sock.groups || [],
        wsState: wsState,
        connectedAt: global.subBotsData?.get(jid)?.connectedAt,
        owner: config?.owner
    }
}
...
export async function deleteAllBots() {
    ...
    for (const [jid, sock] of global.subBots || []) {
        try {
            const num = cleanNum(jid)
            const sessionPath = path.join(SUBBOT_FOLDER, num)
