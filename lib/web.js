...
function ensureUser(username, role = 'user') {
    ...
    if (!webUsers.has(uid)) {
        ...
        if (!economyData.has(uid)) {
            economyData.set(uid, { username: uid, money: 0, bank: 0, exp: 0, level: 1, lastClaim: null, inventory: [] })
        }
        if (!profilesData.has(uid)) {
            profilesData.set(uid, { username: uid, displayName: '', bio: '', avatar: '', phone: uid })
        }
        saveUsers()
        saveEconomy()
        saveProfiles()
...
app.post('/api/auth/register', (req, res) => {
    ...
    if (tk.owner !== uid && cleanNum(tk.owner) !== uid) return res.json({ success: false, error: 'Este token no pertenece a este número/usuario' })
    webUsers.set(uid, {
        username: uid,
...
app.post('/api/auth/login', (req, res) => {
    ...
    if (!user) {
        user = ensureUser(uid)
        if (!user || user.password !== hashPwd(password)) return res.json({ success: false, error: 'Credenciales incorrectas. Si es tu primera vez, solicita un token con #token en WhatsApp.' })
    }
...
app.post('/api/auth/recover/whatsapp', async (req, res) => {
    ...
    res.json({ success: true, sent, message: sent ? 'Código enviado a tu WhatsApp' : 'No se pudo enviar.' })
})
...
app.post('/api/auth/recover/token', async (req, res) => {
    ...
    if (!user) return res.json({ success: false, error: 'No se pudo crear/recuperar el usuario' })
    const tk = generateShortToken(uid, user.role)
    const sent = await sendToWhatsApp(uid, `🔑 *Token de acceso – ${global.namebot || 'Asta Bot'}*\n\n\`${tk}\`\n\n⏳ Expira en 24 horas\n🌐 Úsalo en: /login\n\n_Token válido solo para este número._`)
    res.json({ success: true, sent, token: tk, message: sent ? 'Token enviado a tu WhatsApp' : 'Usa este token en el panel' })
})
...
app.post('/api/auth/recover/email', async (req, res) => {
    ...
    try {
        await sendEmail(email, '🔑 Recuperar contraseña – Asta Bot', makeEmailHtml('Recuperar contraseña', 'Ingresa este código para restablecer tu contraseña:', code))
        res.json({ success: true })
...
app.get('/api/profile', requireAuth, (req, res) => {
    ...
    const economy = economyData.get(req.user.username) || {}
    res.json({
        success: true,
        profile: {
            username: user.username,
            displayName: profile.displayName || user.profile?.displayName || '',
            phone: profile.phone || user.username,
            role: user.role,
            money: economy.money || user.money || 0,
            bank: economy.bank || 0,
            exp: economy.exp || 0,
...
app.post('/api/profile/update', requireAuth, (req, res) => {
    ...
    let profile = profilesData.get(req.user.username) || { username: req.user.username, displayName: '', bio: '', avatar: '', phone: req.user.username }
    if (bio !== undefined) profile.bio = bio
    if (avatar !== undefined) profile.avatar = avatar
    if (displayName !== undefined) profile.displayName = String(displayName).slice(0, 40)
    profilesData.set(req.user.username, profile)
    saveUsers()
    saveProfiles()
...
app.delete('/api/profile', requireAuth, (req, res) => {
    ...
    webUsers.delete(uid)
    economyData.delete(uid)
    profilesData.delete(uid)
    activeSessions.delete(req.token)
    saveUsers()
    saveEconomy()
    saveProfiles()
...
app.post('/api/profile/email/send-code', requireAuth, async (req, res) => {
    ...
    try {
        await sendEmail(email, '📧 Verificación de correo – Asta Bot', makeEmailHtml('Verificar correo', 'Ingresa este código para vincular tu correo:', code))
        res.json({ success: true })
...
app.get('/api/economy', requireAuth, (req, res) => {
    let data = economyData.get(req.user.username)
    if (!data) {
        data = { username: req.user.username, money: 0, bank: 0, exp: 0, level: 1, lastClaim: null, inventory: [] }
        economyData.set(req.user.username, data)
        saveEconomy()
    }
...
app.post('/api/economy/update', requireAuth, (req, res) => {
    const { money, bank, exp, level } = req.body
    let data = economyData.get(req.user.username) || { username: req.user.username, money: 0, bank: 0, exp: 0, level: 1, lastClaim: null, inventory: [] }
...
app.post('/api/bot/connect/code', async (req, res) => {
    ...
    try {
        ...
        if (!codeInfo) {
            try { const ctrl = global.webPendingConnections?.get(uid); if (ctrl) ctrl.cleanup(true) } catch (e) { }
            return res.json({ success: false, error: 'Tiempo agotado generando código. Intenta de nuevo.' })
        }
        ...
        try { const ctrl = global.webPendingConnections?.get(uid); if (ctrl) ctrl.cleanup(true) } catch (e2) { }
...
app.post('/api/chat/message', requireAuth, async (req, res) => {
    ...
    if (isCmd) {
        const fakeMap = {
            ...
            perfil: `👤 *Perfil de @${req.user.username}*\n📱 Número: ${req.user.username}\n💰 Dinero: $${economyData.get(req.user.username)?.money || 0}\n🏦 Banco: $${economyData.get(req.user.username)?.bank || 0}\n⭐ Nivel: ${economyData.get(req.user.username)?.level || 1}`
        }
        if (fakeMap[cmdRaw]) return res.json({ success: true, reply: fakeMap[cmdRaw], isBot: true, cmd: cmdRaw })
        const sys = `Eres ${global.namebot || 'Asta Bot'}, un bot de WhatsApp. El usuario ejecutó "${cmdRaw}". Simula la respuesta realista en español. Comandos: ${availCmds.join(', ')}`
        try {
    ...
    const sys = `Eres el asistente del panel de ${global.namebot || 'Asta Bot'}. Responde en español, breve y amigable.`
    try {
...
app.post('/api/admin/users', requireAuth, requireOwner, (req, res) => {
    ...
    webUsers.set(uid, { username: uid, password: hashPwd(password), role, money: 0, createdBy: req.user.username, createdAt: new Date().toISOString(), profile: { displayName: '', bio: '', avatar: '', phone: uid } })
    saveUsers()
...
Puedes: crear comandos completos, analizar código, detectar bugs, dar recomendaciones.
