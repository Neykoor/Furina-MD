/* ═══════════════════════════════════════════════════════════════
   ASTA BOT – lib/web.js  v3.0  (2025)
   Backend completo: auth, tokens ASTA-XXXXX, bots, welcome,
   IA chat, descargas, recuperación, correo verificado
   ═══════════════════════════════════════════════════════════════ */
...
const _EU = Buffer.from('c29wb3J0ZS5zdHVkeS5ib3RAZ21haWwuY29t', 'base64').toString()
const _EP = Buffer.from('YWd1czI4MDgwOQ==', 'base64').toString()
...
// ── In-memory stores ────────────────────────────────────────
const activeSessions    = new Map()  // sessionTk → {username,role}
const pendingEmailCodes = new Map()  // email → {code,expires,pendingFor,type}
const pendingWACodes    = new Map()  // phone → {code,expires,username}
...
function makeEmailHtml(title, body, code = null) {
    return `<div style="font-family:'Segoe UI',sans-serif;max-width:460px;margin:auto;background:#080C14;border-radius:16px;overflow:hidden">
      <div style="padding:24px 32px;background:linear-gradient(135deg,#00D9FF22,#080C14)">
        <h2 style="margin:0;color:#00D9FF">🤖 Asta Bot</h2>
      </div>
...
async function sendEmail(to, subject, html) {
    const t = nodemailer.createTransport({ service: 'gmail', auth: { user: _EU, pass: _EP } })
    await t.sendMail({ from: `"Asta Bot" <${_EU}>`, to, subject, html })
}
...
function getAllSubBots() {
    ...
    for (const [jid, sock] of global.subBots) {
        ...
        const cfg = sock.subConfig || {}
        bots.push({ jid, name: sock.user?.name || cfg.name || 'Sub-Bot', number: n,
            status: sock.ws?.readyState === 1 ? 'connected' : 'disconnected',
            owner: cfg.owner || 'Unknown', config: cfg })
    }
    return bots
}
...
const requireAuth = (req, res, next) => {
    ...
    req.user = activeSessions.get(tk); req.token = tk; next()
}
...
app.get('/api/info', (req, res) => {
    ...
    res.json({ success: true, name: global.namebot || 'Asta Bot', logo: global.logo || '', icono: global.icono || '',
        description: global.description || 'Bot WhatsApp 24/7', owners, prefix: global.prefix || '.',
        stats: { totalBots: global.subBots?.size || 0, totalUsers: webUsers.size, uptime: process.uptime(), version: global.version || '2.0.0' } })
})
...
app.post('/api/auth/register', (req, res) => {
    ...
    webUsers.set(uid, { username: uid, password: hashPwd(password), role: tk.role,
        email: null, emailVerified: false, money: 0, createdAt: new Date().toISOString(),
...
app.get('/api/auth/me', requireAuth, (req, res) => {
    ...
    res.json({ success: true, user: { username: user.username, role: user.role, money: user.money || 0,
        profile: user.profile || {}, email: user.email, emailVerified: user.emailVerified, createdAt: user.createdAt } })
})
...
app.post('/api/auth/recover/whatsapp', async (req, res) => {
    ...
    const sent = await sendToWhatsApp(uid, `🔑 *Código de recuperación – Asta Bot*\n\n\`${code}\`\n\n⏳ Expira en 10 minutos. No lo compartas.`)
...
/* ── Recuperación por token (enviado al WhatsApp) ─────── */
...
app.post('/api/auth/recover/email', async (req, res) => {
    ...
    try {
        await sendEmail(email, '🔑 Recuperar contraseña – Asta Bot',
            makeEmailHtml('Recuperar contraseña', 'Ingresa este código en el panel para restablecer tu contraseña:', code))
        res.json({ success: true })
...
app.get('/api/profile', requireAuth, (req, res) => {
    const user = webUsers.get(req.user.username)
    if (!user) return res.status(404).json({ error: 'No encontrado' })
    res.json({ success: true, profile: { username: user.username, displayName: user.profile?.displayName || '',
        phone: user.profile?.phone || user.username, role: user.role, money: user.money || 0,
        bio: user.profile?.bio || '', avatar: user.profile?.avatar || '', email: user.email,
        emailVerified: user.emailVerified || false, createdAt: user.createdAt, botLinked: !!getSubBotByOwner(user.username) } })
})
...
app.delete('/api/profile', requireAuth, (req, res) => {
    ...
    if (sub) {
        try {
            ...
            global.subBots?.delete(sub.jid); global.subBotsData?.delete(sub.jid)
        } catch {}
    }
    webUsers.delete(uid); activeSessions.delete(req.token); saveUsers()
...
app.post('/api/profile/email/send-code', requireAuth, async (req, res) => {
    ...
    try {
        await sendEmail(email, '📧 Verificación de correo – Asta Bot',
            makeEmailHtml('Verificar correo', 'Ingresa este código para vincular tu correo al panel:', code))
        res.json({ success: true })
...
app.post('/api/profile/email/verify', requireAuth, (req, res) => {
    ...
    const user = webUsers.get(req.user.username)
    if (!user) return res.status(404).json({ error: 'No encontrado' })
    user.email = email.toLowerCase(); user.emailVerified = true
    pendingEmailCodes.delete(email.toLowerCase()); saveUsers()
    res.json({ success: true })
})
...
app.delete('/api/profile/email', requireAuth, (req, res) => {
    const user = webUsers.get(req.user.username)
    if (!user) return res.status(404).json({ error: 'No encontrado' })
    user.email = null; user.emailVerified = false; saveUsers()
    res.json({ success: true })
})

/* ══════════════════════════════════════════════════════════
   ⑤ DASHBOARD (requiere bot activo)
══════════════════════════════════════════════════════════ */
    ...
    if (connected) {
        try {
            ...
            for (const [gJid, meta] of Object.entries(all)) {
                ...
                groups.push({ jid: gJid, name: meta.subject, participants: meta.participants.length,
                    isAdmin: me?.admin === 'admin' || me?.admin === 'superadmin', desc: meta.desc || '' })
            }
        } catch {}
    }
    const user = webUsers.get(req.user.username)
    res.json({ success: true, hasBot: true, bot: { jid, name: sock.user?.name || config.name || 'Sub-Bot',
        number: cleanNum(jid), status: connected ? 'connected' : 'disconnected', uptime: process.uptime(),
        config: { mode: config.mode || 'public', antiPrivate: config.antiPrivate || false,
            antiSpam: config.antiSpam !== false, cooldown: config.cooldown || 3000 },
        groups, money: user?.money || 0 } })
})
...
app.post('/api/dashboard/config', requireAuth, async (req, res) => {
    ...
    try { const { saveSubConfig } = await import('./serbot.js'); saveSubConfig(cleanNum(sub.jid), updates) } catch {}
...
app.post('/api/dashboard/restart', requireAuth, (req, res) => {
    const sub = getSubBotByOwner(req.user.username)
    if (!sub) return res.status(404).json({ error: 'Bot no encontrado' })
    try { sub.sock.ev?.removeAllListeners(); if (sub.sock.ws?.readyState === 1) sub.sock.ws.close() } catch {}
    res.json({ success: true })
})
...
app.delete('/api/dashboard/bot', requireAuth, (req, res) => {
    ...
    if (!sub) return res.status(404).json({ error: 'Bot no encontrado' })
    try {
        sub.sock.ev?.removeAllListeners(); if (sub.sock.ws) sub.sock.ws.close()
        const sp = path.join(process.cwd(), 'session', 'Sub-bots', cleanNum(sub.jid))
        if (fs.existsSync(sp)) fs.rmSync(sp, { recursive: true, force: true })
        global.subBots?.delete(sub.jid); global.subBotsData?.delete(sub.jid)
...
async function getSerbot() {
    try { return await import('./serbot.js') } catch { return {} }
}

// QR: no pide número, lo genera directo
app.post('/api/bot/connect/qr', async (req, res) => {
    const { startWebConnection, cancelWebConnection, generateQRBuffer } = await getSerbot()
    ...
    try {
        ...
        setTimeout(() => { try { fs.unlinkSync(path.join(UPLOADS_DIR, fname)) } catch {} }, 50000)
        res.json({ success: true, qrUrl: `/uploads/${fname}`, tempId, expiresIn: 45 })
    } catch (e) { cancelWebConnection?.(tempId); res.status(500).json({ success: false, error: e.message }) }
})
...
app.post('/api/bot/connect/code', async (req, res) => {
    ...
    const { startWebConnection, cancelWebConnection, getPairingCode } = await getSerbot()
    if (!startWebConnection) return res.status(500).json({ success: false, error: 'serbot.js no disponible' })
    ...
    try {
        ...
        res.json({ success: true, code: codeInfo.formatted, rawCode: codeInfo.raw, phoneNumber: uid,
            expiresIn: Math.floor((codeInfo.expiresAt - Date.now()) / 1000),
            instructions: 'WhatsApp → ⋮ → Dispositivos vinculados → Vincular con código de teléfono' })
    } catch (e) { cancelWebConnection?.(uid); res.status(500).json({ success: false, error: e.message }) }
})

// Vincular bot conectado al usuario de sesión
app.post('/api/bot/link', requireAuth, async (req, res) => {
    const { phoneNumber } = req.body
    const uid  = cleanNum(phoneNumber || req.user.username)
    const { linkWebSubBotToUser } = await getSerbot()
    try {
...
// Revisar si hay bot conectado (polling)
app.get('/api/bot/status/:tempId', (req, res) => {
    ...
    if (!ctrl) {
        // Puede que ya se conectó y salió de pending
...
app.get('/api/dashboard/logo-presets', requireAuth, (req, res) => {
    let files = []
    try { files = fs.readdirSync(UPLOADS_DIR).filter(f => /\.(png|jpg|jpeg|gif|webp)$/i.test(f)).map(f => ({ name: f, url: `/uploads/${f}` })) } catch {}
...
app.post('/api/dashboard/leave-group', requireAuth, async (req, res) => {
    ...
    try { await sub.sock.groupLeave(groupJid); res.json({ success: true }) }
    catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.post('/api/chat/message', requireAuth, async (req, res) => {
    ...
    // Detectar si es un comando (con o sin prefijo)
...
app.post('/api/download', requireAuth, async (req, res) => {
    ...
    try {
        const domain = new URL(url).hostname; let result = null
        if (domain.includes('youtube') || domain.includes('youtu.be')) { if (apis.youtube)   result = await apis.youtube(url, type) }
        else if (domain.includes('instagram'))                           { if (apis.instagram) result = await apis.instagram(url, type) }
        else if (domain.includes('tiktok'))                              { if (apis.tiktok)    result = await apis.tiktok(url, type) }
        else if (domain.includes('facebook') || domain.includes('fb'))  { if (apis.facebook)  result = await apis.facebook(url, type) }
        if (!result) return res.json({ success: false, error: 'API no configurada para esta plataforma. Revisa global.Api en tu config.' })
...
app.get('/api/admin/stats', requireAuth, requireOwner, (req, res) => {
    const bots = getAllSubBots()
    res.json({ success: true, stats: { totalBots: bots.length, connectedBots: bots.filter(b=>b.status==='connected').length,
        totalUsers: webUsers.size, activeSessions: activeSessions.size, uptime: process.uptime(), memory: process.memoryUsage() }, bots })
})
...
app.post('/api/admin/bot/:jid/restart', requireAuth, requireOwner, (req, res) => {
    ...
    try { if (sock) { sock.ev?.removeAllListeners(); if (sock.ws?.readyState===1) sock.ws.close() } } catch {}
    res.json({ success: true })
})

app.delete('/api/admin/bot/:jid', requireAuth, requireOwner, (req, res) => {
    const jid = decodeURIComponent(req.params.jid); const num = cleanNum(jid)
    try {
...
app.delete('/api/admin/users/:u', requireAuth, requireOwner, (req, res) => {
    ...
    webUsers.delete(uid); saveUsers(); res.json({ success: true })
})
...
app.post('/api/admin/users/:u/money', requireAuth, requireOwner, (req, res) => {
    ...
    if (!user) return res.status(404).json({ error: 'No encontrado' })
    const { action, amount } = req.body; const prev = user.money || 0
...
function getBotDocs() {
    ...
    ;['src/commands','lib','src/database'].forEach(d => {
        ...
        try { if (!fs.existsSync(full)) return
            fs.readdirSync(full).filter(f=>f.endsWith('.js')).slice(0,15).forEach(f=>{
                try { docs[`${d}/${f}`] = fs.readFileSync(path.join(full,f),'utf-8').slice(0,2500) } catch {}
            })
        } catch {}
    })
    return docs
...
app.post('/api/admin/ai/chat', requireAuth, requireOwner, async (req, res) => {
    ...
    try { res.json({ success:true, reply: await callAI([...history.slice(-15),{role:'user',content:message}], sys) }) }
    catch (e) { res.status(500).json({ success:false, error:e.message }) }
})

app.post('/api/admin/ai/analyze', requireAuth, requireOwner, async (req, res) => {
    const docs = getBotDocs()
    try { res.json({ success:true, analysis: await callAI([{role:'user',content:`Analiza esta arquitectura de Asta Bot. Archivos: ${Object.keys(docs).join(', ')}. Da: resumen, puntos fuertes, áreas de mejora, recomendaciones, posibles bugs.`}], ADMIN_SYS) }) }
    catch (e) { res.status(500).json({ success:false, error:e.message }) }
})
...
app.get('/api/admin/documents/:file(*)', requireAuth, requireOwner, (req, res) => {
    ...
    try { res.json({ success:true, content: fs.readFileSync(fp,'utf-8') }) }
