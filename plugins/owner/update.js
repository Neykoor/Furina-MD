...
let handler = async (m, { conn, args, usedPrefix }) => {
  ...
  // Verificar si es owner (usando el isOwner del handler)
  if (!m.isOwner) {
  ...
  try {
    ...
    if (!fs.existsSync(gitDir)) {
      // Intentar inicializar git si no existe
      ...
      try {
        // Inicializar git y configurar el remote
        await execPromise('git init')
        
        // Usar el repositorio global o preguntar
    ...
    // Obtener info de la nueva versión
