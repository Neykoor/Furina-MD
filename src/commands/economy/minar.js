...
let handler = async (m, { conn }) => {
    try {
        ...
        if (!cooldown.ready) {
            return await conn.sendMessage(m.chat, { text: `⛏️ *Mina en enfriamiento*\n\nEspera *${cooldown.remaining}* minutos más` }, { quoted: m })
        }
        ...
        if ((user.level || 1) < mineral.nivel) {
            return await conn.sendMessage(m.chat, { text: `⛏️ *Minaste pero no encontraste nada especial...*\n\nNecesitas nivel ${mineral.nivel} para encontrar ${mineral.nombre}\n💡 Sigue minando para subir de nivel` }, { quoted: m })
        }
