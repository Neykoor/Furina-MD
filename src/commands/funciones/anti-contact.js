...
export async function antiContactDetector(sock, m) {
    const groupJid = m.key?.remoteJid
    if (!groupJid || !groupJid.endsWith('@g.us')) return false
    if (m.key?.fromMe) return false
    if (!isEnabled(groupJid, 'antiContact')) return false

    const tipo = Object.keys(m.message || {})[0]
    if (tipo !== 'contactMessage' && tipo !== 'contactsArrayMessage') return false
    ...
    try {
        ...
        await sock.sendMessage(groupJid, {
            text: `👤 @${senderNum} contactos no permitidos.`,
            mentions: [sender]
        })
        return true
    } catch { return false }
