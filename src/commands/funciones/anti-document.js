...
export async function antiDocumentDetector(sock, m) {
    const groupJid = m.key?.remoteJid
    if (!groupJid || !groupJid.endsWith('@g.us')) return false
    if (m.key?.fromMe) return false
    if (!isEnabled(groupJid, 'antiDocument')) return false

    const tipo = Object.keys(m.message || {})[0]
    if (tipo !== 'documentMessage') return false
    ...
    try {
        ...
        await sock.sendMessage(groupJid, {
            text: `📄 @${senderNum} documentos no permitidos.`,
            mentions: [sender]
        })
        return true
    } catch { return false }
