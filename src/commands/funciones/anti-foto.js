...
export async function antiFotoDetector(sock, m) {
    const groupJid = m.key?.remoteJid
    if (!groupJid || !groupJid.endsWith('@g.us')) return false
    if (m.key?.fromMe) return false
    if (!isEnabled(groupJid, 'antiFoto')) return false

    const tipo = Object.keys(m.message || {})[0]
    if (tipo !== 'imageMessage') return false
    ...
    try {
        ...
        await sock.sendMessage(groupJid, {
            text: `📷 @${senderNum} fotos no permitidas.`,
            mentions: [sender]
        })
        return true
    } catch { return false }
