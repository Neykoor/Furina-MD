...
export async function antiLocationDetector(sock, m) {
    const groupJid = m.key?.remoteJid
    if (!groupJid || !groupJid.endsWith('@g.us')) return false
    if (m.key?.fromMe) return false
    if (!isEnabled(groupJid, 'antiLocation')) return false

    const tipo = Object.keys(m.message || {})[0]
    if (tipo !== 'locationMessage' && tipo !== 'liveLocationMessage') return false
    ...
    try {
        ...
        await sock.sendMessage(groupJid, {
            text: `📍 @${senderNum} ubicaciones no permitidas.`,
            mentions: [sender]
        })
        return true
    } catch { return false }
