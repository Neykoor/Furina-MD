...
import { getFamilyData, adoptChild, removeChild, buildFamilyTree, formatFamilyTree, getRelationLabel, getFamilyStats } from '../../../lib/family.js'
...
let handler = async (m, { conn, args, command }) => {
    try {
        ...
        if (command === 'family' || command === 'familia') {
            if (!args || args.length === 0) {
                const tree = buildFamilyTree(userId)
                if (!tree || (tree.children.length === 0 && tree.parents.length === 0 && !tree.spouse)) {
            ...
            if (args[0] === 'tree' || args[0] === 'arbol') {
                ...
                if (!tree) {
                    return await conn.sendMessage(m.chat, { text: `📭 Árbol vacío.` }, { quoted: m })
                }
            ...
            if (args[0] === 'adopt' || args[0] === 'adoptar') {
                if (!args[1]) {
                    return await conn.sendMessage(m.chat, { text: `❌ Uso: #family adopt @usuario` }, { quoted: m })
                }
                ...
                await conn.sendMessage(m.chat, { text: txt, mentions: [userId + '@s.whatsapp.net', targetId + '@s.whatsapp.net'] }, { quoted: m })
                return
            }

            if (args[0] === 'disown' || args[0] === 'rechazar') {
                if (!args[1]) {
                    return await conn.sendMessage(m.chat, { text: `❌ Uso: #family disown @usuario` }, { quoted: m })
                }
                ...
                if (!family.children.includes(targetId)) {
                    return await conn.sendMessage(m.chat, { text: `❌ Esa persona no es tu hijo.` }, { quoted: m })
                }
            ...
            if (args[0] === 'relation' || args[0] === 'relacion') {
                if (!args[1]) {
                    return await conn.sendMessage(m.chat, { text: `❌ Uso: #family relation @usuario` }, { quoted: m })
                }
                ...
                if (!targetId) {
                    return await conn.sendMessage(m.chat, { text: `❌ Menciona a alguien.` }, { quoted: m })
                }
                ...
                await conn.sendMessage(m.chat, { text: txt, mentions: [targetId + '@s.whatsapp.net'] }, { quoted: m })
                return
            }
        }
...
handler.help = ['family', 'family adopt @user', 'family disown @user', 'family tree', 'family stats']
